	...
	class ActivityEventListener : BaseEntityEventListener
	{
		...
		private static int CountOverlappingActivityEsq(Entity activity)
		{
			...
			IEntitySchemaQueryFilterItem activityIdFilter =
				esqResult.CreateFilterWithParameters(
					FilterComparisonType.NotEqual, 
						"Id", activity.GetTypedColumnValue<Guid>("Id"));
			...
			var dateFilter1 = new EntitySchemaQueryFilterCollection(
				esqResult,
				Terrasoft.Common.LogicalOperationStrict.Or,
				esqResult.CreateFilterWithParameters(FilterComparisonType.Between,
				"StartDate", start, due),
				esqResult.CreateFilterWithParameters(FilterComparisonType.Between, 
				"DueDate", start, due)
			);
			...
			var dateFilter2 = new EntitySchemaQueryFilterCollection(
				...
				esqResult.CreateFilterWithParameters(FilterComparisonType.LessOrEqual,
				"StartDate", start),
				esqResult.CreateFilterWithParameters(FilterComparisonType.GreaterOrEqual,
				"DueDate", due)
			);
